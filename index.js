class Deck {
    ...
    createDeck() {
      ...
      const suits = ["spades ♠️", "clubs ♣️", 'hearts ❤️', "diamonds ♦️"];
    ...
    playGame() {
      ...
      for (let i = 0; i < 26; i++) {
        if (this.player1.hand[i].value > this.player2.hand[i].value) {
          console.log(
            `player1 draws ${this.player1.hand[i].name} of ${this.player1.hand[i].suit}`
          );
          console.log(
            `player2 draws ${this.player2.hand[i].name} of ${this.player2.hand[i].suit}`
          );
  ...
  class Player {
    ...
    //take card from deck push to players hand
    takeCard(card) {
      this.hand.push(card);
    }
    //plays card from players hand
    playCard() {
      return this.hand.shift();
    }
    //point accumulator to caclulate score
    addPoints(points) {
      this.points += points;
    }
    getPoints() {
